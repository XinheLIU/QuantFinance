...
\begin{itemize}
	...
	$$ \mathbf{X} =  \mathbf{a} +  \mathbf{BZ} +  \mathbf{U}$$ 
	want $cov(\mathbf{Z}, \mathbf{U})$ to be 0 and $var{U}$ diagonal,
	want $\mathbf{V} = var(\mathbf{X}) = \mathbf{B}^T var(\mathbf{Z}) \mathbf{B}^T + \mathbf{\Delta}$
	Example: the APT Model
\end{itemize}

From a factor portfolio

\begin{itemize}
	\item Signal portfolio: from factor signals, From a fractile(divide the universe to fractile, form equal weighed portfolios for each fractile) or long-short( long equal weight portfolio with high score, short with low score)
	\item Factor Mimicking Portfolios: with only exposure to one of alpha singal/factor attributes $\mathbf{a}$
	$$ min  \mathbf{h}^T  \mathbf{V}  \mathbf{h}$$, $$ \mathbf{a}^T  \mathbf{h} = 1$$
...
Grinold and Kahn’s rule of thumb
...
Backoffice and Risk Management (See Risk Management chapter)
...
\begin{itemize}
\item Equities $log(\frac{V_{t+1}}{V_t}$ i.i.d
