...
\chapter{Monitoring and Performance Attribution}
...
Measure Performance Steps
\begin{itemize}
 \item Measure returns: Time-weighted(TWRR) or money-weighted (MWRR)
 \item Choose Benchmark: Customized, Broad Indices, Market Indices, Factor Models, Manager Universe or absolute return (Long-short risk neutral fund)
 \item Performance Attribution (next section)
\end{itemize}
...
\begin{itemize}
 \item Macro Performance Attribution
 \item Micro Performance Attribution
 \item Style Analysis
 ...
 (notice, not minimize squared error, use $t-T$ period to estimate $w_i$, then get the selection return(specific risk) \\
  ...
  \end{itemize} 
  Then decompose the risk with factors: the risk of k-th exposure weighted factor:
  $$\sqrt{\frac{1}{T}\sum_{l=1}^T (w_k f_k(l) - \overline{w_k f_k})^2}$$
